...
class PreAE(nn.Module):
    def __init__(self,
        ...
        condition_encoder_dims =[128,128,128,128]
        prev_d = np.sum(class_sizes)
        #self.hidden_dims = hidden_dims  # need this for ODE interaction!
        enc_layers = []
        for i in range(len(condition_encoder_dims)):
            enc_layers.append(nn.Sequential(nn.Linear(prev_d, condition_encoder_dims[i]), nn.ReLU()))
            prev_d = condition_encoder_dims[i]
        self.condition_encoder = nn.Sequential(*enc_layers)
        
        #self.encoder = torch.load("pretrain/lr54.wd0.1024.2.2.bert.ep199.82gene.pth")
        #self.encoder = BERT(hidden=1024,n_layers=2,attn_heads=2,seq_len=82)
        ...
        decoder_dims = [1024,1024,1024,1024]
        prev_d = condition_encoder_dims[-1]+np.sum(class_sizes)#+latent_dim
        dec_layers = []
    ...
    def forward(self, y: Tensor, cats: List[Tensor], **kwargs) -> List[Tensor]:
        ...
        z = self.encode(y.unsqueeze(1),cats)
    ...
    def generate(self, y: Tensor, old_cats: List[Tensor], new_cats: List[Tensor], **kwargs) -> Tensor:
        ...
        z = self.encode(y.unsqueeze(1),old_cats)
