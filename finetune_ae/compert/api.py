    ...
    def evaluate_r2(
        ...
        if spearman:
            scores = pd.DataFrame(columns=[self.covars_key,
                                        self.perturbation_key,
                                        self.dose_key,
                                        'spearmanr_mean', 'spearmanr_mean_DE', 'spearmanr_var',
                                        'spearmanr_var_DE', 'num_cells'])
        else:
            scores = pd.DataFrame(columns=[self.covars_key,
                                            self.perturbation_key,
                                            self.dose_key,
        ...
        for pert_category in np.unique(dataset.pert_categories):
            ...
            if len(idx) > 0:
                ...
                if spearman:
                    mean_score, _ = spearmanr(yt_m, yp_m)
                    var_score, _ = spearmanr(yt_v, yp_v)

                    mean_score_de, _ = spearmanr(yt_m[de_idx], yp_m[de_idx])
                    var_score_de, _ = spearmanr(yt_v[de_idx], yp_v[de_idx])
                else:
...
def evaluate_r2_random(datasets):
    return
