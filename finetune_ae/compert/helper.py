    ...
    for cov_cat in cov_categories:
        ...
        if largestd:
            ...
            for group in de_genes:
                adata_group = adata[adata.obs['cov_drug']==group]
                highstd_index = np.argsort(np.std(adata_group.X.A,axis=0))[-1000:]
                highstd_genes = adata.var.id[highstd_index].index.tolist()
                de_genes_group = de_genes[group].tolist()
                de_genes_std = [gene for gene in de_genes_group if gene in highstd_genes]
                gene_dict[group] = de_genes_std[:n_genes]
        else:
            #compute DEGs
            sc.tl.rank_genes_groups(
                adata_cov,
