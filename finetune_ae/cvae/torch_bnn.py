...
from torch.distributions import MultivariateNormal, Normal, Bernoulli, kl_divergence
...
def get_act(act='relu'):
    ...
    else:                       raise NotImplementedError('Unknown activation {}'.foramt(act))


class BNN(nn.Module):
    def __init__(self, n_in: int, n_out: int, n_hid_layers: int=2,
                 act: str='softplus', dropout=0.0, n_hidden: int=100,
                 requires_grad=True, logsig0=-3, bnn=True, layer_norm=False):
        super().__init__()
        ...
        for i, (n_in, n_out) in enumerate(zip(layers_dim[:-1], layers_dim[1:])):
            ...
            self.acts.append(get_act(act) if i < n_hid_layers else get_act('linear'))  # no act.
